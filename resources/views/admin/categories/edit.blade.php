...
<div class="max-w-4xl space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-7 bg-[#3D3732] rounded-2xl shadow-2xl border border-[#4E4640] overflow-hidden">
            ...
            <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="p-6 sm:p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="description" 
                              ...
                              oninput="updatePreview()"
                              class="w-full px-4 py-3 rounded-xl border text-white placeholder-[#BBAE9F] bg-[#24201D] transition-all duration-200 focus:outline-none focus:bg-[#2A2522] focus:ring-2 focus:ring-[#72383D]/40 focus:border-[#72383D] @error('description') border-rose-500 bg-rose-950/30 @else border-[#4E4640] @enderror">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-xs font-semibold text-rose-400 flex items-center gap-1">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                            {{ $message }}
        ...
        <div class="lg:col-span-5 space-y-4">
            <div class="bg-[#24201D] rounded-2xl p-6 text-white shadow-2xl border border-[#4E4640] relative overflow-hidden">
                ...
                <div class="flex items-center justify-between mb-4">
                    <span class="text-xs font-bold uppercase tracking-wider text-[#E2C599] flex items-center gap-1.5">
                        ...
                        Pratinjau Kartu (Live)
                    </span>
                    <span class="text-[10px] text-[#BBAE9F] bg-[#3D3732] px-2 py-0.5 rounded-md font-mono border border-[#4E4640]">ID #{{ $category->id }}</span>
                </div>
                ...
                <div class="bg-[#3D3732] rounded-xl p-5 border border-[#4E4640] space-y-3 transition-all">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span class="text-[11px] text-[#E2C599] font-medium">
                                {{ $category->products_count ?? $category->products()->count() }} Produk Terdaftar
                            </span>
                        </div>
                    </div>
                    <p id="preview-desc" class="text-xs text-[#BBAE9F] line-clamp-3 leading-relaxed">
                        {{ old('description', $category->description) ? old('description', $category->description) : 'Tidak ada deskripsi.' }}
                    </p>
                </div>

                <div class="mt-4 pt-3 border-t border-[#4E4640] flex items-center justify-between text-[11px] text-[#BBAE9F]">
                    <span>Slug: <strong class="text-[#E2C599] font-mono">{{ $category->slug }}</strong></span>
            ...
            <div class="bg-[#3D3732] border border-[#4E4640] rounded-2xl p-5 text-[#E8D5B7] space-y-2">
                ...
                <p class="text-xs text-[#BBAE9F] leading-relaxed">
                    Mengubah nama kategori tidak akan merusak relasi produk yang sudah ada di dalamnya.
                </p>
            </div>
        </div>
...
<script>
    function updatePreview() {
        ...
        document.getElementById('preview-desc').innerText = descInput ? descInput : 'Tidak ada deskripsi.';
    }
</script>
@endsection
